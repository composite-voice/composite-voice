...
**A browser SDK for building AI voice agents.** Wire together any Speech-to-Text, Language Model, and Text-to-Speech provider behind one unified interface — and swap any of them with a one-line change.
...
const agent = new CompositeVoice({
  stt: new NativeSTT(),
  llm: new AnthropicLLM({ apiKey, model: 'claude-haiku-4-5' }),
  tts: new NativeTTS(),
});
...
That's it. Microphone management, turn-taking, state machines, audio streaming — all handled.

---

## Why CompositeVoice?

Building a voice agent from scratch means solving a lot of hard problems in parallel: microphone capture, WebSocket reconnections, turn-taking logic, interleaving STT/LLM/TTS lifecycles, and wiring together provider SDKs that have nothing in common.

CompositeVoice handles all of that. You describe the pipeline; the SDK runs it.

- **Provider-agnostic.** Deepgram, Anthropic, OpenAI, the browser's built-in APIs — mix and match. Swap providers with a one-line config change.
- **Type-safe throughout.** Every event, config option, and provider method is fully typed. TypeScript autocomplete works end-to-end.
- **Zero mandatory dependencies.** Provider SDKs are optional peer dependencies. Install only what you use.
- **Conversation memory.** Multi-turn history that grows and trims automatically.
- **Eager LLM generation.** Start generating a response before the user finishes speaking, cutting perceived latency.
- **Server-side proxy.** Keep API keys off the client entirely — the SDK includes proxy middleware for Express, Next.js, and plain Node.js.
...
```bash
pnpm add @anthropic-ai/sdk    # for AnthropicLLM
pnpm add @deepgram/sdk        # for DeepgramSTT and DeepgramTTS
pnpm add openai               # for OpenAILLM
pnpm add ws                   # for the server-side proxy (Node.js only)
```
...
Uses the browser's built-in Web Speech API and SpeechSynthesis. Only needs an Anthropic key. Works in Chrome and Edge.
...
Real-time WebSocket STT, the fastest Claude model, and streaming TTS at 24 kHz:
...
|----------|-----------|-------|
| `NativeSTT` | Browser Web Speech API | No API key required. Chrome/Edge only. |
| `DeepgramSTT` | WebSocket | Real-time streaming. Requires `@deepgram/sdk`. Works in all modern browsers. |
...
new NativeSTT({
  ...
  startTimeout: 5000,       // ms to wait for first result before erroring
})
```
...
| Provider | Transport | Notes |
|----------|-----------|-------|
| `NativeTTS` | Browser SpeechSynthesis | No API key required. Works in Chrome, Edge, Safari, Firefox. |
...
agent.once('event.name', handler);                  // one-shot listener
```
...
|-------|---------|-------------|
| `agent.ready` | `{ state }` | SDK initialized and ready to listen |
...
| `error` | Recoverable — call `startListening()` to retry |
...
Each completed turn is automatically appended to the history:
...
With Deepgram v2 models (e.g. `flux-general-en`), the SDK can start LLM generation *before* the user finishes speaking, using an early `preflight` end-of-turn signal. This cuts perceived latency noticeably.
...
const agent = new CompositeVoice({
  ...
  eagerLLM: {
    enabled: true,
    cancelOnTextChange: true,  // restart if the preflight text differed from speech_final
  },
});
```
...
| `aggressive` | Never pause — for hardware echo cancellation setups |
...
Extend the abstract base classes to plug in any provider. The SDK wires it into the pipeline automatically.
...
class MySTT extends BaseSTTProvider {
  ...
  async stopCapture(): Promise<void> { /* stop the stream */ }
}
```
...
Five standalone Vite apps in [`examples/`](./examples/). Each introduces one new concept and builds on the previous:

| # | Stack | What it shows | API keys | Port |
|---|-------|---------------|----------|------|
| [00](./examples/00-native-anthropic-native/) | NativeSTT + Anthropic + NativeTTS | Minimal setup — free STT and TTS, one API key | Anthropic | 3000 |
...
| [04](./examples/04-proxy-server/) | + server proxy | API keys stay on the server, `proxyUrl` in browser | Server-side only | 3004 |
...
Contributions are welcome — bug fixes, new providers, documentation improvements, and feature requests all appreciated.
...
- [GitHub Discussions](https://github.com/lukeocodes/composite-voice/discussions) — questions and ideas
