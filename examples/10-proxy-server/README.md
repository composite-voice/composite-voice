...
cp examples/10-proxy-server/sample.env examples/10-proxy-server/.env
```
...
```bash
pnpm example:10-proxy-server:dev
```
...
pnpm example:10-proxy-server:build

# 2. Start the Express proxy server
cd examples/10-proxy-server
npx tsx server.ts
```

The Express server listens on port 3010, serves static files from `dist/`, and proxies all `/proxy/*` requests with credentials injected by `createExpressProxy`.
...
- Add `conversationHistory` from Example 01 — works identically with `proxyUrl`
- Enable the `eagerLLM` pipeline from Example 21 — works with `proxyUrl` too
