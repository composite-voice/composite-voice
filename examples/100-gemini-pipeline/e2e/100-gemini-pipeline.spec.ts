/**
 * E2E test for example 100-gemini-pipeline.
 *
 * Provider stack: DeepgramSTT (nova-3) + GeminiLLM (gemini-2.0-flash) + ElevenLabsTTS (eleven_turbo_v2_5)
 *
 ...
 *
 * Required environment variables: DEEPGRAM_API_KEY, GEMINI_API_KEY, ELEVENLABS_API_KEY
 */
...
const EXAMPLE_NAME = '100-gemini-pipeline';
...
test.describe('100-gemini-pipeline E2E', () => {
  test('page renders without console errors and key UI elements present', async () => {
    ...
    // Check title
    await expect(page).toHaveTitle(/100.*Gemini.*Pipeline/i);

    // Provider badges
    await expect(page.locator('.badge.stt')).toContainText('Deepgram');
    await expect(page.locator('.badge.llm')).toContainText('Gemini');
