/**
 * E2E test for example 12-custom-provider.
 *
 * Provider stack: NativeSTT (mocked) + MockLLM (custom BaseLLMProvider) + NativeTTS (mocked)
 *
 * Both NativeSTT and NativeTTS are mocked via addInitScript — the Web Speech
 * API is not available in headless Chromium. The MockLLM runs entirely in the
 * browser with zero API keys by cycling through predefined responses.
 *
...
const EXAMPLE_NAME = '12-custom-provider';
...
test.describe('12-custom-provider E2E', () => {
  test('page renders without console errors and key UI elements present', async () => {
    ...
    // Check title
    await expect(page).toHaveTitle(/12.*Custom.*Provider/i);

    // Provider info (tagline shows NativeSTT · MockLLM · NativeTTS)
    await expect(page.locator('.tagline')).toContainText('MockLLM');

    // No-API-key banner
    await expect(page.locator('.banner')).toContainText('Zero API keys');
    ...
    // Token delay config
    await expect(page.locator('#delay-select')).toBeVisible();
  ...
  test('source code viewer toggles visibility on click', async () => {
    const browser = await launchBrowser();
    const context = await createContext(browser, { baseURL: BASE_URL });
    const page = await context.newPage();
    ...
    // Source code should be hidden initially
    const sourceCode = page.locator('#source-code');
    await expect(sourceCode).not.toHaveClass(/visible/);

    // Click toggle to show source code
    await page.locator('#source-toggle').click();
    await expect(sourceCode).toHaveClass(/visible/);

    // Source code should contain the MockLLM class
    await expect(sourceCode).toContainText('class MockLLM extends BaseLLMProvider');
  ...
  test('conversation round-trip: mocked STT → MockLLM → mocked TTS', async () => {
    ...
    try {
      await withRetry(async (attempt, timeoutMs) => {
        ...
        try {
          ...
          // ── LLM: wait for MockLLM response ──────────────────────────
          // MockLLM cycles through predefined responses — first one is:
          // "I'm a mock LLM running entirely in your browser — no API keys needed!"
          ...
          // Verify it's one of the MockLLM predefined responses
          expect(responseText).toMatch(
            /mock LLM|BaseLLMProvider|LLM backend|SDK doesn't care|cycle through/i,
          );
