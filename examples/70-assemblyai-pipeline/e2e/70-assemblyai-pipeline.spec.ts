/**
 * E2E test for example 70-assemblyai-pipeline.
 *
 * Provider stack: AssemblyAISTT + AnthropicLLM (claude-sonnet) + DeepgramTTS (aura-2-thalia-en)
 *
 ...
 * getUserMedia so AssemblyAI receives real audio data over its WebSocket.
 *
 * Required environment variables: ASSEMBLYAI_API_KEY, ANTHROPIC_API_KEY, DEEPGRAM_API_KEY
 */
...
const EXAMPLE_NAME = '70-assemblyai-pipeline';
...
test.describe('70-assemblyai-pipeline E2E', () => {
  test('page renders without console errors and key UI elements present', async () => {
    ...
    // Check title
    await expect(page).toHaveTitle(/70.*AssemblyAI.*Pipeline/i);

    // Provider badges
    await expect(page.locator('.badge.stt')).toContainText('AssemblyAI');
    ...
    try {
      await withRetry(async (attempt, timeoutMs) => {
        ...
        try {
          ...
          // AssemblyAI processes the fake audio capture; transcription text
