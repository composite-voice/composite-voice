/**
 * E2E test for example 90-cartesia-pipeline.
 *
 * Provider stack: DeepgramSTT (nova-3) + GroqLLM (llama-3.3-70b-versatile) + CartesiaTTS (sonic-2)
 *
 ...
 *
 * Required environment variables: DEEPGRAM_API_KEY, GROQ_API_KEY, CARTESIA_API_KEY
 */
...
const EXAMPLE_NAME = '90-cartesia-pipeline';
...
test.describe('90-cartesia-pipeline E2E', () => {
  test('page renders without console errors and key UI elements present', async () => {
    ...
    // Check title
    await expect(page).toHaveTitle(/90.*Cartesia.*Pipeline/i);

    // Provider badges
    await expect(page.locator('.badge.stt')).toContainText('Deepgram');
    await expect(page.locator('.badge.llm')).toContainText('Groq');
    await expect(page.locator('.badge.tts')).toContainText('Cartesia');
