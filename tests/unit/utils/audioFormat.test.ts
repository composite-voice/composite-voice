...
describe('detectAudioFormat', () => {
  ...
  describe('FLAC detection', () => {
    it('detects FLAC from fLaC magic bytes', () => {
      const flacHeader = buf(
        ...
        0x00, 0x00, 0x00, 0x22, // STREAMINFO block header (34 bytes)
        0x00, 0x00, 0x00, 0x00,
      );
  ...
  describe('AAC ADTS detection', () => {
    it('detects AAC from ADTS sync word', () => {
      ...
      const aacAdts = buf(
        0xff, 0xf1, // ADTS sync + flags
        0x50, 0x80, // profile, sample rate, channels
        0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
      );
...
describe('extractHeader', () => {
  describe('WAV', () => {
    it('extracts 44-byte WAV header', () => {
      ...
      [0x57, 0x41, 0x56, 0x45].forEach((b, i) => { view[8 + i] = b; });
  ...
  describe('OGG', () => {
    it('extracts first OGG page up to second OggS marker', () => {
      ...
      [0x4f, 0x67, 0x67, 0x53].forEach((b, i) => { oggData[i] = b; });
      // Second OggS at offset 28
      [0x4f, 0x67, 0x67, 0x53].forEach((b, i) => { oggData[28 + i] = b; });
  ...
  describe('FLAC', () => {
    it('extracts STREAMINFO metadata block', () => {
      ...
      // "fLaC"
      [0x66, 0x4c, 0x61, 0x43].forEach((b, i) => { flacData[i] = b; });
  ...
  describe('AIFF', () => {
    it('extracts 12-byte FORM header', () => {
      ...
      [0x46, 0x4f, 0x52, 0x4d].forEach((b, i) => { aiffData[i] = b; });
      [0x41, 0x49, 0x46, 0x46].forEach((b, i) => { aiffData[8 + i] = b; });
  ...
  describe('WebM', () => {
    it('extracts first 64 bytes as WebM header heuristic', () => {
      ...
      [0x1a, 0x45, 0xdf, 0xa3].forEach((b, i) => { webmData[i] = b; });
  ...
  describe('MP4', () => {
    it('extracts ftyp box using size from first 4 bytes', () => {
      ...
      mp4Data[0] = 0x00; mp4Data[1] = 0x00; mp4Data[2] = 0x00; mp4Data[3] = 0x18;
      // "ftyp"
      [0x66, 0x74, 0x79, 0x70].forEach((b, i) => { mp4Data[4 + i] = b; });
    ...
    it('returns null when buffer is smaller than ftyp box size', () => {
      ...
      mp4Data[0] = 0x00; mp4Data[1] = 0x00; mp4Data[2] = 0x00; mp4Data[3] = 0x64;
      [0x66, 0x74, 0x79, 0x70].forEach((b, i) => { mp4Data[4 + i] = b; });
    ...
    it('returns null when ftyp box size is too small', () => {
      ...
      mp4Data[0] = 0x00; mp4Data[1] = 0x00; mp4Data[2] = 0x00; mp4Data[3] = 0x04;
      [0x66, 0x74, 0x79, 0x70].forEach((b, i) => { mp4Data[4 + i] = b; });
